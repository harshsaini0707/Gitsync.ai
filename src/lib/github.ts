...
//genrate new token of classic version ->link eacuse to increase late limit
...
const[owner , repo] = githubUrl.split('/').slice(-2) // last two items
if(!owner || !repo){
...
const sortedCommit =  data.sort((a: any, b: any) =>
...
export const pollCommits =  async(projectId : string )=>{
 ...
 const summaryResponses = await Promise.allSettled(
  unprocessedCommits.map(commit => { return summariseCommit(githubUrl, commit.commitHash)})
);
